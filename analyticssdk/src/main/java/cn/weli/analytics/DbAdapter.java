...
/* package */ class DbAdapter {
    ...
    private static final String TAG = "WELI.AnalyticsDbAdapter";
    ...
    private long getMaxCacheSize(Context context) {
        try {
            return AnalyticsDataAPI.sharedInstance(context).getMaxCacheSize();
        } catch (Exception e) {
            e.printStackTrace();
            return 32 * 1024 * 1024;
        }
    ...
    public DbAdapter(Context context, String dbName) {
        ...
        mUri = Uri.parse("content://" + dbName + ".AnalyticsDataContentProvider/" + Table.EVENTS.getName());
    }
    ...
    /**
     * 从数据库表中获取limit条数据，并以array形式返回及id
     * @param table
     * @param limit
     * @return
     */
...
//                                j.put("_flush_time", System.currentTimeMillis());
            ...
            LogUtils.i(TAG, "Could not pull records for AnalyticsData out of database " + tableName
                    + ". Waiting to send.", e);
            last_id = null;
            data = null;
        } finally {
